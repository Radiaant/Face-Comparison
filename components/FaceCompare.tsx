  ...
  return (
    <div className="w-full sm:w-1/2 p-2">
      ...
      >
        ...
        <input id={id} type="file" className="hidden" accept="image/png, image/jpeg, image/jpg" onChange={onFileChange} />
      </label>
    </div>
  );
};
    ...
    <div className="mt-8 p-6 bg-gray-800 border border-gray-700 rounded-lg w-full animate-fade-in">
      ...
      <div className="mb-4">
        <div className="flex justify-between mb-1">
          ...
          <span className={`text-sm font-medium ${textColor}`}>{result.similarityPercentage.toFixed(2)}%</span>
        </div>
        <div className="w-full bg-gray-700 rounded-full h-2.5">
          <div className={progressColor + " h-2.5 rounded-full"} style={{ width: `${result.similarityPercentage}%` }}></div>
        </div>
      </div>
      <div>
        <h4 className="font-semibold text-gray-300 mb-2">AI Reasoning:</h4>
        <p className="text-gray-400 text-sm bg-gray-900 p-3 rounded-md">{result.reasoning}</p>
      </div>
    </div>
  );
};
...
const FaceCompare = () => {
    ...
    const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>, setImage: React.Dispatch<React.SetStateAction<File | null>>) => {
        ...
        <div className="bg-gray-800/50 p-4 sm:p-8 rounded-xl shadow-2xl border border-gray-700">
            <form onSubmit={handleSubmit}>
                <div className="flex flex-col sm:flex-row -m-2">
                    <ImageUploadBox id="file1" image={image1} onFileChange={(e) => handleFileChange(e, setImage1)} title="Upload Image 1" />
                    <ImageUploadBox id="file2" image={image2} onFileChange={(e) => handleFileChange(e, setImage2)} title="Upload Image 2" />
                </div>
                <div className="mt-6">
                    <button 
                        type="submit" 
